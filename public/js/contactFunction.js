...
export async function loadChatMessages(append = false) {
    ...
    try {
        const res = await fetch(`/api/messages/${receiverId}?skip=${state.messagesSkip}&limit=${state.messagesLimit}`, {
            method: 'GET', credentials: 'include'
        });
        const data = await res.json();
        const oldScrollHeight = messagesContainer.scrollHeight;
        if (data.messages.length < state.messagesLimit) state.allMessagesLoaded = true;
